...
app.post('/createPost', upload.single('file'), async (req, res) => {
  ...
  if (!category_name || category_name.trim() === '') {
    return res.status(400).send({ error: 'El nombre de la categoría no puede estar vacío.' });
  }

  try {
    // Verifica si la categoría ya existe
    
    const categoryNameLower = category_name.trim().toLowerCase();
    
    const categoryQuery = 'SELECT * FROM category WHERE category_name = $1';
    let category = await pool.query(categoryQuery, [categoryNameLower]);

    if (category.rows.length === 0) {
      // Si no existe, crea la categoría
      const insertCategoryQuery = 'INSERT INTO category (category_name) VALUES ($1) RETURNING *';
      const newCategory = await pool.query(insertCategoryQuery, [categoryNameLower]);
      category = newCategory.rows[0]; // Toma el nuevo ID de categoría
    } else {
      category = category.rows[0]; // Usa el ID de la categoría existente
    }

    // Cambia la ruta para almacenar en la base de datos
    const filePath = `uploads/post/${req.file.filename}`; // Almacena la ruta relativa

    // Crea el post usando el id_category de la categoría encontrada o creada
    const postQuery = `
      INSERT INTO post (title, description, creation_date, id_category, id_user, fyle, typefile)
      VALUES ($1, $2, CURRENT_TIMESTAMP, $3, $4, $5, $6)
      RETURNING *;
    `;
    const newPost = await pool.query(postQuery, [title, description, category.id_category, req.session.user.id, filePath, req.file.mimetype]);

    res.status(201).send({ id: newPost.rows[0].id_post });
  } catch (error) {
...
app.get('/categoryList', async (req, res) => {
  try {
    const categoryList = await User.getAllCategories(); // Método para obtener todas las categorías
...
app.post('/filterByCategory', async (req, res) => {
  ...
  if(!id_category) {
    console.log('');
    return res.status(400).json({ message: 'No category query provided' })
  }

  try {
    const filterResults = await User.filterByCategory(id_category)
