...
static async getPostById(decodedId) {
  ...
  static async getOrCreateCategory(categoryName) {
    try {
      // Primero, buscamos si la categoría ya existe
      let result = await pool.query('SELECT id_category FROM category WHERE category_name = $1', [categoryName]);
  
      if (result.rows.length > 0) {
        // Si ya existe, retornamos el ID de la categoría
        return result.rows[0].id_category;
      } else {
        // Si no existe, la creamos
        result = await pool.query('INSERT INTO category (category_name) VALUES ($1) RETURNING id_category', [categoryName]);
        return result.rows[0].id_category; // Devolvemos el ID de la nueva categoría
      }
    } catch (error) {
      console.error('Error al obtener o crear la categoría: ', error);
      throw error;
    }
  }
  ...
  static async filterByCategory(id_category) {
    try {
      const result = await pool.query(
        'SELECT * FROM post WHERE id_category = $1',  // Corrige "id_actegory" a "id_category"
        [id_category]
      );
      return result.rows;
    } catch (error) {
